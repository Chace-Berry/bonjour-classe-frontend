...
function QuizModal({ show, quizId, onClose }) {
  ...
  const handleQuit = () => {
    ...
    onClose(); // Use the onClose prop instead of setShowQuizModal(false)
    // Reset states
    setCurrentQuestion(0);
    setSelectedAnswer(null);
    setQuizQuestions([]);
  ...
  const checkAnswer = async () => {
    if (quizQuestions[currentQuestion]?.type === "sentence_building") {
      ...
      const correct_order =
        quizQuestions[currentQuestion].options.correct_order ||
        Array.from(
          { length: quizQuestions[currentQuestion].options.words.length },
          (_, i) => i
        );
  ...
  const initializeSentenceBuilding = (question) => {
    if (
      ...
      Array.isArray(question.options?.words)
    ) {
    ...
    <div>
      ...
      >
        <Modal.Body className="p-0">
          ...
          >
            ...
            {quizFailed ? (
              ...
              >
                ...
                >
                  You completed {correctQuestions.length} out of{" "}
              ...
              >
                ...
                >
                  You got {correctQuestions.length} out of{" "}
                        ...
                        <DragDropContext
                          onDragEnd={(result) => {
                            ...
                            if (
                              ...
                              destination.droppableId === "sentence-area"
                            ) {
                            ...
                            // Moving from sentence area to word bank
                              ...
                              source.droppableId === "sentence-area" &&
          ...
          >
            <button
              onClick={handleQuit} // Changed from setShowQuitConfirmation(true) to handleQuit
              style={{
                backgroundColor: "rgba(255,255,255,0.2)",
                color: darkMode ? "white" : "black",
                border: "none",
